...
function shouldKeepTempRoot() {
  const value =
    process.env.HAGICODE_NON_INTERACTIVE_INTEGRATION_KEEP_TEMP?.trim().toLowerCase();
...
async function removePathWithRetries(
  targetPath,
  { maxAttempts = 6, retryDelayMs = 500 } = {},
) {
  let lastError = null;
...
async function loadDesktopManagedPathHelpers() {
  const desktopRuntimePathsModulePath = path.join(
    projectRoot,
    "dist",
    "main",
...
function findUnpackedArtifactRoot() {
  ...
  for (const rootName of platformRoots) {
    ...
    if (pathExists(candidate)) {
      log(
        `skipping unpacked artifact candidate without runnable Desktop resources: ${candidate}`,
      );
    }
  }
...
export function findZipArtifact() {
  ...
  const zips = findFilesWithExtension(pkgRoot, ".zip").sort((a, b) => {
...
export function findAppImageArtifact() {
  if (!pathExists(pkgRoot)) {
    return null;
  }

  const appImages = findFilesWithExtension(pkgRoot, ".appimage");

  return appImages[0] ?? null;
}

export function findDmgArtifact() {
  if (!pathExists(pkgRoot)) {
    return null;
  }

  const scoreDmgArtifact = (dmgPath) => {
  ...
  const dmgs = findFilesWithExtension(pkgRoot, ".dmg").sort((a, b) => {
    const scoreDelta = scoreDmgArtifact(b) - scoreDmgArtifact(a);
    if (scoreDelta !== 0) {
      return scoreDelta;
    }
...
function shouldRestoreExecutableBit(filePath) {
  ...
  if (
    normalized.includes("/resources/extra/") &&
    (normalized.includes("/bin/") || path.extname(baseName) === "")
  ) {
    return true;
  }
...
async function extractZipArtifact(zipArtifact, stagedRoot) {
  if (process.platform === "darwin") {
    const result = await runCommand("ditto", [
      "-x",
      "-k",
      zipArtifact,
      stagedRoot,
    ...
    if (result.code !== 0) {
      fail(
        `Failed to extract macOS ZIP artifact with ditto.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
      );
    }
    return;
  }
...
async function extractTarGzArtifact(tarGzArtifact, stagedRoot) {
  ...
  if (result.code !== 0) {
    fail(
      `Failed to extract tar.gz artifact.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    );
  }
}

async function extractAppImageArtifact(appImageArtifact, stagedRoot) {
  await fsp.chmod(appImageArtifact, 0o755);
  const result = await runCommand(appImageArtifact, ["--appimage-extract"], {
    cwd: stagedRoot,
  });
  if (result.code !== 0) {
    fail(
      `Failed to extract AppImage artifact.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    );
  }

  const extractedRoot = path.join(stagedRoot, "squashfs-root");
  if (!pathExists(extractedRoot)) {
    fail(
      `AppImage extraction did not create squashfs-root under ${stagedRoot}.`,
    );
  }
  await restoreExecutablePermissions(extractedRoot);
  return extractedRoot;
}

async function extractDmgArtifact(dmgArtifact, stagedRoot) {
  const mountRoot = path.join(stagedRoot, "mounted-dmg");
  await fsp.mkdir(mountRoot, { recursive: true });
  let attached = false;

  try {
    const attachResult = await runCommand("hdiutil", [
      "attach",
      dmgArtifact,
      "-nobrowse",
      "-readonly",
    ...
    if (attachResult.code !== 0) {
      fail(
        `Failed to mount DMG artifact.\nstdout:\n${attachResult.stdout}\nstderr:\n${attachResult.stderr}`,
      );
    }
    attached = true;

    const appBundle = walkFiles(mountRoot)
      .filter((file) => file.endsWith(".app/Contents/Info.plist"))
      .map((file) => file.slice(0, -"/Contents/Info.plist".length))
      .sort((a, b) => a.length - b.length)[0];

    if (!appBundle) {
      fail(`Mounted DMG does not contain a .app bundle: ${dmgArtifact}`);
    }
...
async function copyArtifactToPathWithSpaces() {
  ...
  if (process.platform === "linux") {
    const appImageArtifact = findAppImageArtifact();
    if (appImageArtifact) {
      const artifactRoot = await extractAppImageArtifact(
        appImageArtifact,
        stagedRoot,
      );
      return { tempRoot, artifactRoot, source: appImageArtifact };
    }
  ...
  if (process.platform === "darwin") {
    const dmgArtifact = findDmgArtifact();
    if (dmgArtifact) {
      const artifactRoot = await extractDmgArtifact(dmgArtifact, stagedRoot);
      return { tempRoot, artifactRoot, source: dmgArtifact };
    }
  }
...
function findWindowsExecutable(root) {
  const candidates = walkFiles(root)
    ...
    .filter(
      (file) => !file.toLowerCase().includes(`${path.sep}resources${path.sep}`),
    )
...
function findLinuxExecutable(root) {
  ...
  const candidates = walkFiles(root)
    ...
    .sort((a, b) => {
      const aPreferred = preferredNames.has(path.basename(a).toLowerCase())
        ? 0
        : 1;
      const bPreferred = preferredNames.has(path.basename(b).toLowerCase())
        ? 0
        : 1;
...
function listRunnableResourceCandidates(root) {
  if (process.platform === "darwin") {
    return walkFiles(root).filter(
      (file) =>
        file.endsWith(
          `${path.sep}Contents${path.sep}Resources${path.sep}app.asar`,
        ) ||
        file.endsWith(
          `${path.sep}Contents${path.sep}Resources${path.sep}app-update.yml`,
        ),
    );
  }
...
function runExecutable(executablePath, userDataDir, commandArgs) {
  const requiresLinuxHeadlessSwitches =
  ...
  return new Promise((resolve) => {
    ...
    const timeoutTimer = setTimeout(() => {
      timedOut = true;
      log(
        `command timeout exceeded after ${commandTimeoutMs}ms, sending SIGTERM`,
      );
      child.kill("SIGTERM");
...
function assertOutputValue(output, label, expected) {
  ...
  if (value !== expected) {
    fail(
      `Expected "${label}" to equal "${expected}" but found "${value ?? "<missing>"}".`,
    );
  }
}
...
function assertPathWithinRoot(candidatePath, rootPath, label) {
  const resolveComparablePath = (targetPath) => {
    ...
    try {
      return (
        fs.realpathSync.native?.(targetPath) ?? fs.realpathSync(targetPath)
      );
    } catch {
  ...
  if (
    ...
    !comparableRootPath ||
    !comparableCandidatePath.startsWith(comparableRootPath)
  ) {
    fail(
      `Expected ${label} to stay under ${rootPath}.\nPath: ${candidatePath ?? "<missing>"}`,
    );
  }
}
...
function assertPathContainsSpaces(candidatePath, label) {
  if (!candidatePath?.includes(" ")) {
    fail(
      `Expected ${label} to preserve staged paths with spaces.\nPath: ${candidatePath ?? "<missing>"}`,
    );
  }
}
...
async function readIntegrationDiagnostics(userDataDir) {
  ...
  for (const targetPath of candidateFiles) {
    ...
    diagnostics.push(
      `== ${path.relative(userDataDir, targetPath) || path.basename(targetPath)} ==\n${content.trim()}`,
    );
  }

  if (diagnostics.length === 0 && pathExists(runtimeDataRoot)) {
    diagnostics.push(
      `runtimeData root exists but no matching diagnostic files were found under ${runtimeDataRoot}`,
    );
  }
...
function assertRuntimeVerificationOutput(
  output,
  { artifactRoot, userDataDir, helpers },
) {
  ...
  if (dataHome !== expectedDataHome) {
    fail(
      `Expected runtime data home to use the migrated ~/.hagicode/runtime-data contract.\nExpected: ${expectedDataHome}\nActual: ${dataHome}`,
    );
  }
...
async function main() {
  ...
  try {
    ...
    if (process.platform === "linux") {
      log(
        "launch mode: Linux sandbox override enabled for staged artifact execution",
      );
    }
    ...
    if (
      ...
      fail(
        "Integration harness must run with artifact and managed paths containing spaces.",
      );
    }
