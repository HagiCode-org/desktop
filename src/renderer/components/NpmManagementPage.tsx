  ...
  const runBatchInstall = async () => {
    ...
    try {
      ...
      setOperationError((current) => {
        const next = { ...current };
        for (const packageId of packageIds) {
          next[packageId] = result.error ?? t('npmManagement.errors.operationFailed');
        }
        return next;
      });
    } catch (error) {
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t('npmManagement.packageTable.status')}</TableHead>
                ...
                <TableBody>
                  {managedPackages.map((item) => {
                    ...
                    return (
                      <TableRow key={item.id} data-state={selectedPackageIds.includes(item.id) ? 'selected' : undefined}>
                        <TableCell>
                          <Checkbox
                        ...
                        <TableCell>
                          <Badge variant={packageBadgeVariant(item.status)}>{t(`npmManagement.packageStatus.${item.status}`)}</Badge>
                        </TableCell>
                        ...
                        <TableCell className="space-y-2 text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button size="sm" variant="outline" onClick={() => void runOperation(item.id, 'uninstall')} disabled={rowDisabled || item.status !== 'installed' || item.definition.required === true}>
