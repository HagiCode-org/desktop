    ...
    <Card className="border-primary/40">
      <CardHeader className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle className="text-lg">{t('npmManagement.bootstrap.title')}: {item.definition.displayName}</CardTitle>
...
interface NpmPackageTableProps {
  ...
  progressByPackageId: Partial<Record<ManagedNpmPackageId, NpmManagementOperationProgress>>;
    ...
    <Card>
      <CardHeader className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground" aria-live="polite">
          {t('npmManagement.selection.selectedCount', { count: selectedEligibleCount })}
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        {!hagiscriptGateOpen && (
