...
当前 Hagico Desktop 在部署 Web 服务时，用户主机环境可能缺少必要的运行时依赖项（如 .NET Runtime）。这导致：
...
```mermaid
flowchart TD
    A[应用启动] --> B[初始化 DependencyManager]
    B --> C[调用 checkAllDependencies]
    C --> D{检测平台}

    D -->|Windows| E[执行 dotnet --list-runtimes]
    D -->|macOS/Linux| F[执行 dotnet --list-runtimes]

    E --> G[解析输出]
    F --> G

    G --> H{版本比较}
    H -->|已安装且版本匹配| I[返回 installed=true]
    H -->|已安装但版本不匹配| J[返回 installed=false, versionMismatch=true]
    H -->|未安装| K[返回 installed=false]
