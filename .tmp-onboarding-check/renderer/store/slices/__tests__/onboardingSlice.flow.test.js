...
import reducer, { getOnboardingSequence, selectCanGoNext, selectLegalDocuments, selectLegalMetadataSource, setDownloadProgress, } from '../onboardingSlice.js';
import { acceptLegalDocuments, checkOnboardingTrigger, goToNextStep, goToPreviousStep, loadLegalDocuments, } from '../../thunks/onboardingThunks.js';
...
describe('onboardingSlice flow', () => {
    ...
    it('opens the legal-only gate on trigger results that require consent only', () => {
        const state = reducer(undefined, checkOnboardingTrigger.fulfilled({
            shouldShow: true,
            mode: 'legal-only',
    ...
    it('advances from welcome to legal consent before sharing acceleration', () => {
        let state = reducer(undefined, checkOnboardingTrigger.fulfilled({
            shouldShow: true,
            mode: 'full',
        ...
        assert.equal(state.currentStep, OnboardingStep.LegalConsent);
        assert.equal(selectCanGoNext({ onboarding: state }), false);
    });
    it('advances from legal consent to sharing acceleration after acceptance', () => {
        ...
        state = reducer(state, goToNextStep());
        state = reducer(state, acceptLegalDocuments.fulfilled({
            mode: 'full',
            locale: 'zh-CN',
        ...
        assert.equal(selectCanGoNext({ onboarding: state }), true);
    });
    it('stores remote or cached legal metadata for the consent step', () => {
        const state = reducer(undefined, loadLegalDocuments.fulfilled(legalDocumentsPayload, 'request-id', { locale: 'zh-CN', refresh: false }));
        assert.equal(selectLegalMetadataSource({ onboarding: state }), 'cache');
        assert.equal(selectLegalDocuments({ onboarding: state }).length, 2);
    });
    it('closes the wizard after legal-only acceptance', () => {
        const initial = reducer(undefined, checkOnboardingTrigger.fulfilled({
            shouldShow: true,
            mode: 'legal-only',
            reason: 'legal-consent-required',
            runtimeProvisioned: true,
        ...
        const accepted = reducer(initial, acceptLegalDocuments.fulfilled({
            mode: 'legal-only',
            locale: 'en-US',
            documents: legalDocumentsPayload.documents.map((document) => ({
                documentType: document.documentType,
    ...
    it('keeps the flow on download after the package is ready so the wizard can finish there', () => {
        ...
        const readyToLaunch = reducer(state, setDownloadProgress({
            progress: 100,
    ...
    it('only enables next on download after the package is ready', () => {
        ...
        state = reducer(state, goToNextStep());
        state = reducer(state, setDownloadProgress({
            progress: 100,
            downloadedBytes: 1024,
            totalBytes: 1024,
            speed: 0,
